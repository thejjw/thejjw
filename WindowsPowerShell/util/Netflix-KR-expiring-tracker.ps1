<#
.SYNOPSIS
    Zero-dependency, self-contained script to scrape expiring Netflix South Korea titles from uNoGS.com.
.DESCRIPTION
    Replicates the entire scraping and rendering logic without requiring Node.js.
    Fetches expiring search results, genres, details, downloads/embeds boxart, and renders a fully interactive HTML dashboard.
.NOTES
    Copyright (c) 2026 @thejjw. All rights reserved.
    Last Updated: June 28, 2026
.EXAMPLE
    .\Netflix-KR-expiring-tracker.ps1
#>
...
$JsonPath = "data\unogs-expiring-kr-$DateSuffix.json"
$HtmlPath = "data\unogs-expiring-kr-$DateSuffix.html"

Write-Host "=== Starting Zero-Dependency Netflix South Korea Expiring Scraper ===" -ForegroundColor Cyan
...
try {
    ...
    # 2. Fetch Expiring Titles
    Write-Host "`nStep 2: Querying expiring titles for South Korea (Country ID 348)..."
    $searchHeaders = @{
        "User-Agent" = $UserAgent
    ...
    $searchRes = Invoke-RestMethod -Uri "https://unogs.com/api/search?query=expiring&countrylist=$CountryCode" -Headers $searchHeaders -Method Get

    if (-not $searchRes -or -not $searchRes.results) {
        Write-Host "No expiring titles returned or search results array is empty." -ForegroundColor Yellow
        $results = @()
    } else {
        $results = $searchRes.results
    }

    Write-Host "Found $($results.Count) expiring titles." -ForegroundColor Green
    ...
    foreach ($res in $results) {
        $count++
        Write-Host " [$count/$($results.Count)] Processing details for: $($res.title)..." -ForegroundColor Gray
        ...
        # B. Fetch details (Maturity Rating, runtime, etc.)
        $detail = $null
        try {
        ...
        # C. Download and embed Boxart image
        $boxartEmbedded = ""
        if ($res.img) {
            $boxartEmbedded = Get-Base64Image -Url $res.img
        }

        # D. Normalize item structure
        ...
        $normalized = [PSCustomObject]@{
            ...
            expiryDate = $res.expires
            expiryText = if ($res.expires) { "Expires on $($res.expires)" } else { "" }
            type = $res.vtype
    ...
    # Convert tz offset format to colon separating hour/minute
    $collectedAt = $collectedAt -replace '(\d{2})(\d{2})$', '$1:$2'

    $outputData = [PSCustomObject]@{
        source = "unogs"
        sourceUrl = "https://unogs.com/countrydetail"
        country = $CountryCode
...
<head>
  ...
  <title>Netflix South Korea - Expiring Soon</title>
...
<body>
  <div class="container">
    <header class="header">
      <h1 class="title">Netflix South Korea &mdash; Leaving Soon</h1>
      <p class="subtitle">Scraped from uNoGS on <span id="generationDate">--</span></p>
    </header>

    <div class="stats-grid">
      <div class="stat-card">
        <div class="stat-val" id="countTotal">0</div>
        <div class="stat-lbl">Total Expiring</div>
      </div>
      <div class="stat-card">
        <div class="stat-val" id="countMovies">0</div>
        <div class="stat-lbl">Movies</div>
      </div>
      <div class="stat-card">
        <div class="stat-val" id="countSeries">0</div>
  ...
  <script>
    ...
    function filterTitles() {
              ...
              <div class="card-top">
                ...
                <div class="card-meta">
                  <div class="badge-row">
                    ...
                    <span class="badge badge-expiry">${escapeHtml(item.expiryText || 'Leaving Soon')}</span>
                  </div>
      ...
      document.getElementById('countMovies').innerText = filtered.filter(i => i.type.toLowerCase() === 'movie').length;
      document.getElementById('countSeries').innerText = filtered.filter(i => i.type.toLowerCase() === 'series').length;
    }
...
'@
    ...
    $FinalHtml = $HtmlTemplate.Replace("/*JSON_DATA_PLACEHOLDER*/", $JsonContent).Replace("/*DEV_DATE*/", $DevDate)
    ...
    # 5. Print terminal summary list
    Write-Host "`nStep 4: Expiring Titles Summary List" -ForegroundColor Cyan
    Write-Host "==========================================================" -ForegroundColor Cyan

    if ($items.Count -eq 0) {
        Write-Host "No expiring titles found for today." -ForegroundColor Gray
    } else {
        foreach ($item in $items) {
            ...
            Write-Host "$($item.expiryDate)" -ForegroundColor Red -NoNewline
    ...
    Write-Host "Total Titles Scraped: $($items.Count)" -ForegroundColor Cyan
    Write-Host "==========================================================" -ForegroundColor Cyan
