...
Read `KIMI_CODE_PLAN_API_KEY` from the environment. On the personal Windows setup, `Load-AiApiKeysFromCS` normally loads it from Windows Credential Manager. Never print the key.
...
- `Authorization: Bearer <KIMI_CODE_PLAN_API_KEY>`

### PowerShell

```powershell
if ([string]::IsNullOrWhiteSpace($env:KIMI_CODE_PLAN_API_KEY)) {
    throw 'KIMI_CODE_PLAN_API_KEY is not set.'
}

$headers = @{
    Accept        = 'application/json'
    Authorization = "Bearer $env:KIMI_CODE_PLAN_API_KEY"
}
...
```bash
: "${KIMI_CODE_PLAN_API_KEY:?KIMI_CODE_PLAN_API_KEY is not set}"

curl --fail-with-body --silent --show-error \
  'https://api.kimi.com/coding/v1/usages' \
  -H 'Accept: application/json' \
  -H "Authorization: Bearer $KIMI_CODE_PLAN_API_KEY"
```
